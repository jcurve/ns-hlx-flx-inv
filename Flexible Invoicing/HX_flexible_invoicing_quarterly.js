...
define([
    ...
    util){
        ...
        var SO_SEARCHID = util.SEARCHID().SOQUARTERLY_SRCH_ID;
        var dummyDate = scriptObj.getParameter({name: 'custscript_flex_inv_dummy_date_quarterly'});
        function getInputData(context){

            //Determine Execution Day
            var param = util.SCRIPTPARAMS().QRTRLY_TESTING;
            
            var isValidDay = util.validExecutionDay(param,'QUARTERLY',dummyDate);
            if (!isValidDay){
                log.debug({
                    title : 'isValidDay',
        ...
        function GetQuarterDays(currDate){
        try{
            ...
            */
            ...
            //determine which quarter to compute based on current month.
            ...
            var runQ = null;
            if (q1.indexOf(currMonth)!=-1)
                runQ = q4;
            if (q2.indexOf(currMonth)!=-1)
                runQ = q1;
            if (q3.indexOf(currMonth)!=-1)
                runQ = q2;
            if (q4.indexOf(currMonth)!=-1)
                runQ = q3;
            
            var daysOfMos = util.DAYSOFMONTH(util.isLeapYear(currYear));
            var totalDays = 0;
            for (q in runQ){
                totalDays+= parseInt(daysOfMos[runQ[q]]);
            }
        ...
        try{
            ...
            var objQtr = GetQuarterDays(dateNow);
                ...
                if (lineIdsToProcess.indexOf(parseInt(recLineId))==-1){
                   ...
                   // log.debug({ title: 'removing',details: 'id: '+soId+', line: ' + recLineId});
                    ...
                    for (jj in objVals){
                        ...
                        rate = calculateRate(objVals[jj],revisedAmt,dateNow,objQtr);
         ...
         */
		function calculateRate(objVal,revisedAmt,currDate,objQtr){
        try{           
            
            var totalDays = objQtr.totalDays;
            var monthEndId = objQtr.monthend;
            var currYear = currDate.getFullYear();
            if (monthEndId==11){//will use previous year, not current year
               currYear =  currYear - 1;
            }
              
            var monthEndDate = new Date(currYear, monthEndId+1, 0);
            ...
            var daysDiff = util.DAYSDIFF(monthEndDate,acDate);
